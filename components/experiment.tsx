		...
		<div>
			{exp.rollout[3].map((population, i) => {
				...
				return <div key={i} style={{background: '#202225', padding: '.5rem', marginBottom: '.25rem', borderRadius: '2px'}}>
					...
					<div style={{color: '#ed4245'}}>{popTotal < 10_000 && <>{exp.data.description[0]}: <strong>{(10_000 - popTotal)/100}%</strong></>}</div>
				</div>
			})}
		</div>
...
const parseFilter = (f: Filter) => {
	...
	if (f[0] === FilterType.RangeByHash) return `Server ID is in range by hash key ${f[1][0][1]} with target ${f[1][1][1]}`
